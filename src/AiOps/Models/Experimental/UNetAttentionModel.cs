...
//		public UNetAttentionModel(IProjectPresenter project, Device device) : base(nameof(UNetAttentionModel))
//		{
...
//			bottleneckLayer = new DoubleConvolutionBuilder($"Bottleneck", inC, filterSizes[depth], para.UseInstanceNorm, para.UseDropout, para.UseChannelAttention, device, precision).Build();

//			var rawHeads = 1 << depth - 1;
//			var numHeads = Math.Min(Math.Max(rawHeads, 2), 8);
//			var bottleneckChannels = filterSizes[depth];
