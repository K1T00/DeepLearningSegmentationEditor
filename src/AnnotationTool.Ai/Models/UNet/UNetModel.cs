...
namespace AnnotationTool.Ai.Models.UNet
{
    ...
    public sealed class UNetModel : Module<Tensor, Tensor>, ISegmentationModel
    {
        ...
        public UNetModel(DeepLearningProject project, Device device, SegmentationModelConfig cfg) : base(nameof(UNetModel))
        {
            this.cfg = cfg;
            var inChannels = project.Settings.PreprocessingSettings.TrainAsGreyscale ? 1 : 3;

            // Binary → [B, 1, H, W]
            // Multiclass → [B, C, H, W]
            var numFeatures = project.Features.Count == 1
                ? 1
                : project.Features.Count + 1; // +1 for background class in multiclass segmentation
            var outChannels = numFeatures == 1 ? 1 : numFeatures;
            ...
            // Output layer
            outLayer = Conv2d(filterSizes[0], outChannels, 1, device: device, dtype: this.cfg.TrainPrecision);
        ...
        /// <summary>
        /// Constructor for SDK usage
        /// </summary>
        /// <param name="settings"></param>
        /// <param name="device"></param>
        /// <param name="cfg"></param>
        /// <param name="numClasses"></param>
        public UNetModel(DeepLearningSettings settings, Device device, SegmentationModelConfig cfg, int numClasses) : base(nameof(UNetModel))
        {
            this.cfg = cfg;
            ...
            // Binary → [B, 1, H, W]
            // Multiclass → [B, C, H, W]
            var numFeatures = numClasses == 1
                ? 1
                : numClasses + 1; // +1 for background class in multiclass segmentation
            var outChannels = numFeatures == 1 ? 1 : numFeatures;
